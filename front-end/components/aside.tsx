...
function Aside({ setActivePage }: { setActivePage: (page: string) => void }) {
  ...
  const SidebarContent = () => (
    <nav className="flex flex-col h-full">
      <ul className="space-y-2 text-[14px]">
        <li
          className="flex items-center space-x-2 bg-[#F1ECFF] p-[12px] rounded-[8px] w-[93%] cursor-pointer"
        ...
        {[
          ...
          <li
            key={idx}
            onClick={() => setActivePage(text.toLowerCase())}
            className="flex items-center space-x-2 p-2 text-gray-700 cursor-pointer"
          >
            {icon}
            {gap ? (
      ...
      <div className="flex items-center mt-auto space-x-2 p-2 text-gray-700 cursor-pointer">
        <Users size={18} />
        <span className="text-[#1D1E20] text-[14px] font-[400]">
          Account Sharing
        </span>
      </div>
    </nav>
